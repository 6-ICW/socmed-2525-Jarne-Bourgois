...
function MessageItem({ msg, currentUser, isAdmin, onDelete, onEdit }) {
  ...
  return (
    <div className="message-row">
      <div className="message-avatar" style={{ background: getColor(msg.author?.username || '?') }}>
      ...
      <div className="message-body">
        <div className="message-header">
          ...
          {msg.is_edited && <span className="message-edited-tag">(bewerkt)</span>}
        </div>
        {editing ? (
          <div className="edit-input-area">
            ...
            <div className="edit-actions">
              ...
              {' · '}
              <span onClick={() => { setEditing(false); setEditContent(msg.content); }}>Annuleren</span>
            </div>
          </div>
        ) : (
      ...
      {(canEdit || canDelete) && !editing && (
        <div className="message-actions">
          {canEdit && (
            <button className="msg-action-btn" onClick={() => setEditing(true)} title="Bewerken">✏️</button>
          )}
          {canDelete && (
            <button
              className={`msg-action-btn delete${isAdmin && !isMine ? ' admin-delete-btn' : ''}`}
              onClick={() => onDelete(msg.id)}
              title={isAdmin && !isMine ? 'Admin: bericht verwijderen' : 'Verwijderen'}
            >
              🗑️
            </button>
          )}
...
export default function ChatArea({ channel, user, currentUserRole }) {
  ...
  const editMessage = (messageId, content) => {
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      wsRef.current.send(JSON.stringify({ action: 'edit_message', message_id: messageId, content }));
    }
  };
